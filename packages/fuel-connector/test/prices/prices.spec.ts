...
describe("Prices contract", () => {
  ...
  async function performPayloadTest(
    callback: (
      contract: PricesContract,
      payloadData: string[]
    ) => Promise<PricesOutput>
  ): Promise<number[]> {
    const contract = await getPricesContract();
    const payloadData = readTestData("2sig_ETH_BTC");

    const value = await callback(contract, payloadData);

    return value.map((res: U256Output) => {
  ...
  it("get_prices should return the price data", async () => {
    const values = await performPayloadTest(
      async (contract: PricesContract, payloadData: string[]) => {
