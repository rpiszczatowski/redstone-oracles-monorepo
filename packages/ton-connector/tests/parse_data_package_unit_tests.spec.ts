...
const DATA_PACKAGE_VALUES_LENGTH =
  DATA_FEED_ID_BS + DEFAULT_NUM_VALUE_BS + TIMESTAMP_BS;
...
describe("TON parse data package Unit Tests", () => {
  ...
  it("parse data package should fail for wrong data", async () => {
    expect(
      testerAdapter.testParseDataPackage("00" + DATA_PACKAGE_DATA_1)
    ).rejects.toHaveProperty("exitCode", 1000);

    expect(testerAdapter.testParseDataPackage("")).rejects.toHaveProperty(
      "exitCode",
      9
    );
