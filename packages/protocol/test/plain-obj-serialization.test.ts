...
const prepareMultiSignDataPackageForTests = (
  dataPoints: DataPoint[]
): MultiSignDataPackage => {
  const dataPackage = new DataPackage(dataPoints, TIMESTAMP_FOR_TESTS);
  return dataPackage.multiSign([
...
describe("Fixed size data package", () => {
  ...
  test("Should correctly serialize multi signed standard numeric data package", () => {
    const dataPoints = [
      { dataFeedId: "BTC", value: 20000 },
      { dataFeedId: "ETH", value: 1000 },
    ];
    const signedDataPackage = prepareMultiSignDataPackageForTests(
      dataPoints.map((dp) => new NumericDataPoint(dp))
    );
    const serializedPlainObj = signedDataPackage.toObj();
    expect(serializedPlainObj).toEqual({
  ...
  test("Should correctly serialize multi signed data package with mixed values", () => {
    const dataPoints: DataPoint[] = [];
    dataPoints.push(new NumericDataPoint({ dataFeedId: "ETH", value: 1000 }));
    dataPoints.push(
      new NumericDataPoint({
    ...
    const signedDataPackage = prepareMultiSignDataPackageForTests(dataPoints);
    const serializedPlainObj = signedDataPackage.toObj();
    expect(serializedPlainObj).toEqual({
      dataPoints: [
        { dataFeedId: "ETH", value: 1000 },
      ...
      signatures: [
        "WJ+EFIe6pSwzCRcjKWIYMWyhmtKJP+tN2aI55+Ip5w4osIGH0ngUEjTO4b7sAPBGd5MIv11qbPaFxWReppbGDRs=",
        "iVfU2iRNiNSblSqjCzJ7KaACnSpTccHbWsa+BGTTXcIwYvP6Ae90apjRU/FCA1s7oRjTnLN0ZAKN5QvQKbpfLxw=",
      ],
    });
