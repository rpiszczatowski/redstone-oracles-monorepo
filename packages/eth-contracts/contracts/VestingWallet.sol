...
contract VestingWallet {
    ...
    function getLockedAmount(uint256 timestamp) public view virtual returns (uint256) {
        if (timestamp < start + cliffDuration) {
            ...
            return allocation * (start + cliffDuration + vestingDuration - timestamp) / vestingDuration;
        }
    }    
    ...
    function getReleasable() public view virtual returns (uint256) {
        return token.balanceOf(address(this)) < getLockedAmount(block.timestamp) ? 0            
    ...
    function stake(uint256 amount) external {
        require(token.balanceOf(address(this)) >= amount, "VestingWallet: there is not enough tokens to stake");
